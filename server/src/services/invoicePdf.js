...
export const buildInvoicePdf = async (booking, options = {}) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    // Terms (fixed near bottom)
    if (settings.invoiceTerms) {
      doc.font("Helvetica").fontSize(8).fillColor("#8b97b3").text(settings.invoiceTerms, 46, 770, { width: 498 });
    }
    doc.font("Helvetica").fontSize(9).fillColor("#57658a").text(
      `Generated by ${settings.businessName || "OasisGo Holidays"} · System invoice — valid without signature`,
      46, 800, { width: 498, align: "center" }
    );
