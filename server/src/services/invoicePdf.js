  ...
  return new Promise((resolve, reject) => {
    ...
    if (footerY < 740) {
      ...
      doc.font("Helvetica").fontSize(9).fillColor("#8b97b3").text(
        `Thank you for choosing ${settings.businessName || "OasisGo Holidays"}.`,
        46, footerY, { width: 498, align: "center" }
      );
      footerY += 14;
    }

    // Terms + generated line — pinned near the bottom but safely within the
    // page so they never trigger an overflow page. `lineBreak:false` stops
    // PDFKit from paginating a long single line.
    if (settings.invoiceTerms) {
      doc.font("Helvetica").fontSize(8).fillColor("#8b97b3").text(settings.invoiceTerms, 46, 758, { width: 498, lineBreak: false, ellipsis: true, height: 20 });
    }
    doc.font("Helvetica").fontSize(9).fillColor("#57658a").text(
      ...
      46, 780, { width: 498, align: "center", lineBreak: false }
    );
